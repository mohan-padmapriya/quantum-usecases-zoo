...
const HomeBody: QuartzComponent = ({ fileData, allFiles, tree }: QuartzComponentProps) => {
  ...
  return (
    <div class="usecase-home">
      <header class="usecase-home-header">
        <div class="usecase-home-header-text">
          <h1>{title}</h1>
          {description && <p class="usecase-home-thesis">{description}</p>}
        </div>
        {
          // Reuses the darkmode plugin's existing global CSS/script (it's
          // bundled site-wide regardless of layout slot) — this page just
          // can't place the plugin's own button since it clears left/right.
        }
      ...
      <nav class="usecase-home-actions">
        <a
          href={resolveRelative(fileData.slug!, simplifySlug("about"))}
              ...
              <div class="filter-group" data-filter-key={group.key}>
                ...
                {group.values.map((value) => (
                  <button class="pill" data-value={value}>
                    {group.key === "verdict" ? getVerdictMeta(value).label : titleCase(value)}
                  </button>
                ))}
              </div>
            ),

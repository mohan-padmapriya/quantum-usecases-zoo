...
The most advanced classical approach to date is a 2026 study by Zhai et al, and utilises a filtering funnel of classical heuristics and high-level coupled cluster theory to achieve near perfect accuracy. However, the strong electronic correlation and near-degenerate energy levels of the cofactor results in the classical resource requirements to scale exponentially as the system size and required precision increase. 

Quantum computing could perhaps be advantageous here as it encodes many-body wavefunctions directly onto an entangled register of qubits, which could in theory allow for ground-state energy estimation with polynomial scaling, bypassing the classical curse of dimensionality. Consequently, FeMoco serves as a fundamental benchmark for demonstrating practical quantum utility in chemistry.

# Gates of quantum advantage
## Theoretical quantum advantage
The theoretical case for exponential speedup via quantum phase estimation is heavily conditional on the ability to prepare an initial state with non-negligible ground-state overlap. Unfortunately, systematic evaluations have found that this overlap generically decays as system sizes grow. On the other hand, classical heuristics remain competitive, which is why there is still some debate in the community about whether there is enough evidence for such an exponential advantage across chemical space.  Even proponents at Google have conceded that asymptotic advantage is a "subject of legitimate debate," shifted their focus toward finite-instance performance. The 2026 classical solution by Zhai et al. also suggested that the FeMoco ground state is "only a slightly entangled state" and that the primary difficulty lies in ranking candidate states, a task classical filtering funnels can now handle to the required accuracy. 

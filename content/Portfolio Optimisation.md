...
When researcher try to prove quantum advantage, in order make these problems "quantum-ready," researchers use "penalties" to enforce rules (like selecting exactly (K) assets). These penalties make the problem artificially dense, causing current quantum hardware to fail because the device spends all its energy enforcing rules rather than finding the best stocks. 
...
And then of course there is the practical deployment on cloud-based quantum systems, which inevitably introduces lot of latency. While the quantum chip itself might compute in milliseconds, the end-to-end process can take minutes or even hours.
